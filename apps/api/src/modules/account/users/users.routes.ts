...
export function userRoutes() {
  ...
  router.get(
    ...
    describeRoute({
      ...
      description: "The authenticated user's profile and group memberships.",
      security: SECURITY.session,
      responses: {
        ...jsonOk(userSchema.extend({ groups: z.array(z.unknown()) }), "Current user"),
        ...errors(401),
      },
    }),
    async (c) => {
  ...
  router.get(
    ...
    describeRoute({
      ...
      security: SECURITY.session,
      responses: {
        ...jsonOk(z.array(z.unknown()), "Groups"),
        ...errors(401),
      },
    }),
    async (c) => {
  ...
  router.get(
    ...
    describeRoute({
      ...
      security: SECURITY.session,
      responses: {
        ...jsonOk(z.object({ key: z.string(), value: z.string() }).nullable(), "Preference"),
        ...errors(401),
      },
    }),
    async (c) => {
  ...
  router.put(
    ...
    describeRoute({
      ...
      security: SECURITY.session,
      responses: {
        ...jsonOk(z.null(), "Preference saved"),
        ...errors(401, 422),
      },
    }),
    async (c) => {
  ...
  router.get(
    ...
    describeRoute({
      ...
      security: SECURITY.session,
      responses: {
        ...jsonOk(z.array(z.unknown()), "TOTP devices"),
        ...errors(401),
      },
    }),
    async (c) => {
  ...
  const createTotpSchema = z.object({ name: z.string().min(1).max(100) });
  ...
  router.post(
    ...
    describeRoute({
      ...
      security: SECURITY.session,
      responses: {
        ...jsonCreated(z.unknown(), "TOTP device created"),
        ...errors(401, 422),
      },
    }),
    validator("json", createTotpSchema),
    async (c) => {
      const db = c.get("db");
  ...
  router.post(
    ...
    describeRoute({
      ...
      description: "Verifies the device's first code. Requires a fresh step-up token once any device is verified.",
      security: SECURITY.session,
      responses: {
        ...jsonOk(z.null(), "TOTP device confirmed"),
        ...errors(400, 401, 429),
      },
    }),
  ...
  router.delete(
    ...
    describeRoute({
      ...
      description: "Requires a fresh step-up token once any device is verified.",
      security: SECURITY.session,
      responses: {
        ...jsonOk(z.null(), "TOTP device deleted"),
        ...errors(401, 404),
      },
    }),
    async (c) => {
  ...
  router.post(
    ...
    describeRoute({
      tags: [TAGS.Account],
      summary: "Verify TOTP and issue step-up token",
      security: SECURITY.session,
      responses: {
        ...jsonOk(z.object({ token: z.string() }), "Step-up token"),
        ...errors(400, 401, 422, 429),
      },
    }),
    rateLimit({ windowMs: 5 * 60 * 1000, max: 10, bucket: "totp-stepup" }),
    validator("json", z.object({ code: z.string().length(6) })),
    async (c) => {
      const db = c.get("db");
      const user = c.get("user")!;
      const body = c.req.valid("json");
  ...
  router.get(
    ...
    describeRoute({
      ...
      description: "Directory of active users for sharing/assignment pickers. Any authenticated user.",
      security: SECURITY.session,
      responses: {
        ...jsonOk(z.array(z.unknown()), "Visible users"),
        ...errors(401),
      },
    }),
    async (c) => {
  ...
  router.get(
    ...
    describeRoute({
      tags: [TAGS.Account],
      summary: "List users",
      description: "Paginated, filterable user list. Admin only.",
      security: SECURITY.session,
      responses: {
        ...jsonPaged(userSchema, "Users"),
        ...errors(401, 403, 422),
      },
    }),
    adminRequired,
  ...
  router.patch(
    ...
    describeRoute({
      tags: [TAGS.Account],
      summary: "Update user",
      description: "Change a user's role and/or status. Admin only; cannot modify own account.",
      security: SECURITY.session,
      responses: {
        ...jsonOk(userSchema, "User updated"),
  ...
  router.get(
    ...
    describeRoute({
      ...
      security: SECURITY.session,
      responses: {
        ...jsonOk(z.array(z.unknown()), "User groups"),
        ...errors(401, 403, 404),
      },
    }),
    adminRequired,
